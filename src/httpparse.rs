        ...
        match pr {
            httparse::Status::Complete((pos, headers)) => {
                ...
                for header in headers {
                    let name = HeaderName::from_bytes(header.name.as_bytes()).expect("header name validated by httparse");
                    let value_start = header.value.as_ptr() as usize - bytes_ptr;
                    let value_end = value_start + header.value.len();
                    let value = unsafe { HeaderValue::from_maybe_shared_unchecked(bytes.slice(value_start..value_end))};
